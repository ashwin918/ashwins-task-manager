    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                // Step 3: Wait for DB — use ping instead of timeout (works in Jenkins bat)
                bat """
                    echo Waiting for task-manager-db...
                    docker exec %DB_CONTAINER% pg_isready -U postgres 2>nul || ping -n 4 127.0.0.1 >nul
                    docker exec %DB_CONTAINER% pg_isready -U postgres 2>nul || ping -n 4 127.0.0.1 >nul
                    docker exec %DB_CONTAINER% pg_isready -U postgres 2>nul || ping -n 4 127.0.0.1 >nul
                    docker exec %DB_CONTAINER% pg_isready -U postgres 2>nul || ping -n 4 127.0.0.1 >nul
                    docker exec %DB_CONTAINER% pg_isready -U postgres 2>nul || ping -n 4 127.0.0.1 >nul
